...
export default async function(req: Request): Promise<Response> {
  ...
  try {
    ...
    if (endDate < startDate) { await failOperation(base44, opResult.operation.id, "end_before_start"); return Response.json({ error: "end_date must be on or after start_date" }, { status: 400 }); }
    // Derive new status: if end_date is today or past, status=expired; otherwise stays active until it passes
    const today = new Date(); today.setHours(0, 0, 0, 0);
    const endDay = new Date(endDate); endDay.setHours(0, 0, 0, 0);
    const derivedStatus = endDay <= today ? "expired" : "active";
