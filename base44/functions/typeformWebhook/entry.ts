...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ============================================
    // STEP 1: VERIFY SIGNATURE (enforced — rejects anonymous forged submissions)
    // ============================================
    ...
    if (!signatureHeader) {
      console.error(`[Webhook ${requestId}] Missing typeform-signature header`);
    ...
    const a = encoder.encode(receivedHash);
    const b = encoder.encode(computedHash);
    if (a.length !== b.length) {
      console.error(`[Webhook ${requestId}] Signature length mismatch`);
