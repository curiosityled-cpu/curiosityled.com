...
export default async function(req: Request): Promise<Response> {
  ...
  const { operation_id, cycle_id, new_status } = body;
  ...
  try {
    ...
    const auditEvent = await writeSuccessionAuditEvent({
      base44, action_type: "cycle_stage_updated",
      target_entity_type: "SuccessionCycle", target_entity_id: cycle_id,
      metadata: { new_status },
      operation_id, event_key: { action: "cycle_updated", cycle_id, new_status },
