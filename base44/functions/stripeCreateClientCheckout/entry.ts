...
Deno.serve(async (req) => {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: sessionMode,
      success_url: `${appUrl}/BusinessManager?success=true&session_id={CHECKOUT_SESSION_ID}&client_id=${client.id}`,
      cancel_url: `${appUrl}/BusinessManager?canceled=true&client_id=${client.id}`,
      metadata: {
        client_id: client.id,
        client_slug: client.slug,
