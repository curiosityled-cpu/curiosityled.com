...
export default async function(req: Request): Promise<Response> {
  ...
  const authz = await authorizeSuccessionAction({ base44, auth, action: "successionReturnBlueprint", target_client_id: auth.client_id, required_permission: "succession.roles.manage" });
  if (!authz.allowed) return Response.json({ error: authz.denied_reason }, { status: 403 });
  const opResult = await createOrAttachOperation({ base44, client_id: auth.client_id, operation_id, function_name: "successionReturnBlueprint", payload: { blueprint_id }, actor_profile_id: auth.profile_id, actor_email: auth.email, actor_context_type: auth.isPlatformAdmin ? "platform_operator" : "tenant" });
