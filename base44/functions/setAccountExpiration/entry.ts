...
Deno.serve(async (req) => {
  try {
    ...
    const { userId, expirationDate, accountType, notifyUser = true } = await req.json();
    ...
    return Response.json({
      success: true,
      message: 'Account expiration updated successfully',
      userId,
      expirationDate,
      accountType
    });
    ...
    console.error('Error setting account expiration:', error);
    return Response.json({
      error: 'Failed to set account expiration',
