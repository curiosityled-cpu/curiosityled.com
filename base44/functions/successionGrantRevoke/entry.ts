...
/**
 * POST /successionGrantRevoke
 *
 * Control-plane function for revoking a cross-tenant access grant.
  ...
  try {
    ...
    if (!isGrantFeatureEnabled()) {
      await writeSuccessionAuditEvent({
        base44,
        action_type: "grant_revoke_disabled",
    ...
    await writeSuccessionAuditEvent({
      base44,
      action_type: "grant_revoke_attempted",
      actor_context_type_override: "platform_operator",
      metadata: { note: "Phase 0 stub — full workflow not yet implemented" },
    });

    return Response.json(
      { enabled: true, message: "Grant revoke path is enabled but the full workflow is not yet implemented in Phase 0." },
      { status: 200 }
    );
  } catch (error) {
