...
export default async function(req: Request): Promise<Response> {
  ...
  const { operation_id, org_position_id, title, position_identifier, reports_to_position_id, reason } = await req.json().catch(() => ({}));
  ...
  try {
    ...
    if (reportsToChanged) {
      await base44.asServiceRole.entities.OrgPositionChange.create({ client_id: auth.client_id, org_position_id, change_type: "reports_to_changed", previous_reports_to_position_id: position.reports_to_position_id || null, new_reports_to_position_id: reports_to_position_id, changed_by_profile_id: auth.profile_id, changed_at: new Date().toISOString(), reason: reason || "Reporting line updated", operation_id: opResult.operation.id, confidentiality_level: "confidential", integrity_status: "active" });
    }
