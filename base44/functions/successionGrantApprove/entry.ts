...
/**
 * POST /successionGrantApprove
 *
 * Control-plane function for approving a cross-tenant access grant.
  ...
  try {
    ...
    if (!isGrantFeatureEnabled()) {
      await writeSuccessionAuditEvent({
        base44,
        action_type: "grant_approve_disabled",
    ...
    await writeSuccessionAuditEvent({
      base44,
      action_type: "grant_approve_attempted",
      actor_context_type_override: "platform_operator",
      metadata: { note: "Phase 0 stub — full workflow not yet implemented" },
    });

    return Response.json(
      { enabled: true, message: "Grant approve path is enabled but the full workflow is not yet implemented in Phase 0." },
      { status: 200 }
    );
  } catch (error) {
