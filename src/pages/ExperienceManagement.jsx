...
import { useAuth as useFullAuth } from "@/components/useAuth";
import { useNavigate } from "react-router-dom";
import {
  Users, AlertTriangle, TrendingUp, CheckCircle, Loader2, Search,
  ChevronRight, Map, BookOpen, Plus, X, Bookmark, Wrench, Inbox
} from "lucide-react";
...
function CategoryManagerList({ managers, allInsights, onClick }) {
  return (
...
const TABS = [
  ...
  { id: 'journeys', label: 'Journeys & Programs' },
];
...
export default function ExperienceManagement() {
  ...
  const { data: allInsights = {}, isLoading: loadingInsights } = useQuery({
    queryKey: ['all-insights', clientId],
    queryFn: async () => {
  ...
  return (
    <MVPPageLayout
      ...
      subtitle="Monitor manager development and assign learning programs."
      action={
    ...
    >
        ...
        <>
          {isLoading ? (
            ...
            <>
              ...
              {/* Summary Stats */}
              <div className="grid grid-cols-3 gap-3">
                {statCards.map(({ key, label, value, icon: Icon, iconBg, iconColor, activeBg }) => (
                  <Card
                    key={key}
      ...
      {activeTab === 'journeys' && (
        <Suspense fallback={<div className="flex justify-center p-12"><Loader2 className="w-8 h-8 animate-spin text-[#0202ff]" /></div>}>
