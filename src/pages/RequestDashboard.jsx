...
import ProgramAdminPerformance from "../components/requests/ProgramAdminPerformance";
import RequestAgentChat from "../components/requests/RequestAgentChat";
import PublicRequestLinkGenerator from "../components/requests/PublicRequestLinkGenerator";
import RequestStatistics from "../components/requests/RequestStatistics";
import AssigneeKanban from "../components/requests/AssigneeKanban";
import AdvancedFilters from "../components/requests/AdvancedFilters";
import ProgramAdminCard from "../components/program-admin/ProgramAdminCard";
import RequestAssignmentPanel from "../components/program-admin/RequestAssignmentPanel";
import AIAssignmentRecommender from "../components/program-admin/AIAssignmentRecommender";

function RequestDashboard() {
  const { user, isHRAdmin, isProgramManager, isAnyAdmin, isPlatformAdmin } = useAuth();
  ...
  const [viewMode, setViewMode] = useState('table'); // 'table', 'kanban', or 'assignee'
  ...
  const [activeSubtab, setActiveSubtab] = useState('requests'); // 'requests' or 'admins'
  ...
  const loadRequests = async () => {
    setLoading(true);
    try {
      // Load legacy DevelopmentRequest records
      const legacyRequests = await base44.entities.DevelopmentRequest.list('-created_date');
      
      // Load new form-based requests (CustomFormSubmissions where form_type='request')
      ...
      if (formIds.length > 0) {
        ...
        // Transform submissions to request format
    ...
    try {
      ...
      if (response.data?.success) {
        ...
        const enrichedAdmins = await Promise.all(admins.map(async (admin) => {
          const assignedRequests = requests.filter(r => r.assigned_to_email === admin.email);
          
          const upcomingClasses = await base44.entities.Class.filter({
            facilitator_emails: { $in: [admin.email] },
            status: { $in: ['scheduled', 'in_progress'] }
          });

          const coachingSessions = await base44.entities.CoachingSession.filter({
          ...
          return {
            ...admin,
            workload: {
              active_requests: assignedRequests.filter(r => !['completed', 'cancelled'].includes(r.status)).length,
              upcoming_classes: upcomingClasses.length,
              coaching_sessions: coachingSessions.length,
              active_goals: goals.length,
              total_requests: assignedRequests.length,
              overdue_requests: assignedRequests.filter(r => 
                r.due_date && new Date(r.due_date) < new Date() && !['completed', 'cancelled'].includes(r.status)
              ).length
            }
          };
        }));
        
        setProgramAdmins(enrichedAdmins);
      }
    } catch (error) {
  ...
  const statistics = useMemo(() => {
    ...
    return {
      ...
      breachingSLA: requests.filter(r => 
        r.status === 'new' && 
        !r.first_response_at &&
  ...
  const uniqueAssignees = useMemo(() => {
    ...
    requests.forEach(r => {
      if (r.assigned_to_email) assignees.add(r.assigned_to_email);
    });
  ...
  const filteredAdmins = programAdmins.filter(admin =>
    ...
    admin.specializations?.some(s => s.toLowerCase().includes(adminSearchTerm.toLowerCase()))
  );
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-start gap-2 mb-6">
            ...
            {isHRAdmin && (
              <Button
                onClick={() => setShowPublicLinkGenerator(!showPublicLinkGenerator)}
                variant="outline"
                size="sm"
              >
                Public Link
            ...
            <Button
              variant={viewMode === 'assignee' ? 'default' : 'outline'}
        ...
        {/* Tabs for HR Admins: Requests + Program Admins */}
        {isHRAdmin ? (
          <Tabs value={activeSubtab} onValueChange={setActiveSubtab} className="mt-6">
            <TabsList>
              ...
              <TabsTrigger value="admins" className="gap-2">
                <Users className="w-4 h-4" />
                My Program Admins ({programAdmins.length})
              </TabsTrigger>
            </TabsList>
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-4 mt-6">
          ...
          <Card>
            <CardContent className="p-4 text-center">
              ...
              <p className="text-2xl font-bold">{statistics.awaitingApproval}</p>
          ...
          <Card className="border-2 border-red-200">
            <CardContent className="p-4 text-center">
              ...
              <p className="text-2xl font-bold">{statistics.breachingSLA}</p>
              <p className="text-xs text-gray-600">SLA Breach (72h)</p>
            </CardContent>
          </Card>
          ...
          <Card className="border-2 border-orange-200">
            <CardContent className="p-4 text-center">
              ...
              <p className="text-2xl font-bold">{statistics.stale}</p>
        ...
        {(isHRAdmin || isAnyAdmin) && programAdmins.length > 0 && (
          <div className="mt-6">
            <ProgramAdminPerformance requests={requests} programAdmins={programAdmins} />
          </div>
        )}

        {/* Requests Table, Kanban, or Assignee View */}
        {viewMode === 'assignee' ? (
          <div className="mt-6">
            <AssigneeKanban
              ...
              onAssigneeChange={async (requestId, newAssigneeEmail) => {
                try {
                  await base44.entities.DevelopmentRequest.update(requestId, {
                    assigned_to_email: newAssigneeEmail,
                    status: newAssigneeEmail ? 'assigned' : 'new'
                  });
                  await loadRequests();
              ...
              <Table>
                ...
                <TableBody>
                  {processedRequests.map(request => {
                    ...
                    const isStale = ['assigned', 'in_progress'].includes(request.status) &&
                      new Date(request.updated_date) < new Date(Date.now() - 14 * 24 * 60 * 60 * 1000);
                    const breachesSLA = request.status === 'new' && 
                      !request.first_response_at &&
                      new Date(request.created_date) < new Date(Date.now() - 3 * 24 * 60 * 60 * 1000);
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <Card>
                    <CardContent className="p-4 text-center">
                      ...
                      <p className="text-2xl font-bold">{programAdmins.length}</p>
                      <p className="text-xs text-gray-600">Program Admins</p>
                    </CardContent>
                  </Card>
                  ...
                  <Card>
                    <CardContent className="p-4 text-center">
                      ...
                      <p className="text-2xl font-bold">
                        {programAdmins.reduce((sum, a) => sum + (a.workload?.active_requests || 0), 0)}
                      </p>
                  ...
                  <Card>
                    <CardContent className="p-4 text-center">
                      ...
                      <p className="text-2xl font-bold">
                        {programAdmins.reduce((sum, a) => sum + (a.workload?.upcoming_classes || 0), 0)}
                      </p>
                      <p className="text-xs text-gray-600">Upcoming Classes</p>
                    </CardContent>
                  </Card>

                  <Card className={programAdmins.some(a => (a.workload?.overdue_requests || 0) > 0) ? "border-2 border-red-200" : ""}>
                    <CardContent className="p-4 text-center">
                      <AlertCircle className={`w-6 h-6 mx-auto mb-2 ${
                        programAdmins.some(a => (a.workload?.overdue_requests || 0) > 0) ? 'text-red-600' : 'text-gray-400'
                      }`} />
                      <p className="text-2xl font-bold">
                        {programAdmins.reduce((sum, a) => sum + (a.workload?.overdue_requests || 0), 0)}
                      </p>
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <p className="font-medium text-purple-900">
                              {unassignedRequests.length} Unassigned Request{unassignedRequests.length !== 1 ? 's' : ''}
                            </p>
                            <p className="text-sm text-purple-700 mt-1">
                              Get AI-powered assignment recommendations based on expertise and workload
                            </p>
                          </div>
                        </div>
                        <Button
                  ...
                  <CardContent>
                    <div className="grid md:grid-cols-2 gap-4">
                      {filteredAdmins.map(admin => (
                        <ProgramAdminCard key={admin.id} admin={admin} />
                      ))}
                    </div>
                    ...
                    {filteredAdmins.length === 0 && (
                      <div className="text-center py-12">
                        ...
                        <p className="text-gray-600">
                          {adminSearchTerm ? 'No admins match your search' : 'No Program Admins assigned yet'}
                        </p>
                      </div>
                    )}
                  </CardContent>
        ...
        <RequestAgentChat
          minimized={!showAgent}
          onMinimize={() => setShowAgent(!showAgent)}
        />
        ...
        <Dialog open={showPublicLinkGenerator} onOpenChange={setShowPublicLinkGenerator}>
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>Generate Public Submission Link</DialogTitle>
            </DialogHeader>
...
export default withAuthProtection(RequestDashboard, {
