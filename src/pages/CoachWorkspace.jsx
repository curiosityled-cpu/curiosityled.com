...
import { COACHING_CATEGORIES, STATUS_CONFIG } from "@/components/coaching-request/shared";
...
export default function CoachWorkspace() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Only coaching-category requests assigned to me
      const mine = all.filter(
        (r) => r.assigned_practitioner_email === user.email && COACHING_CATEGORIES.includes(r.request_category)
      );
      setRequests(mine);
    ...
    <div className="p-4 md:p-8 max-w-5xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Review assigned coaching requests, accept or decline, and manage your active engagements.
        </p>
      </div>
            ...
            active.map((e) => (
              <div key={e.id} className="p-4 rounded-xl border border-border bg-card">
                ...
                <p className="text-xs text-muted-foreground">
                  {e.coachee_email || e.team_member_emails?.join(", ") || "Team engagement"}
                </p>
              </div>
            ))
          )}
