...
const RADAR_DATA = [
  { area: "Perf. Response", value: 62 },
  { area: "Coaching", value: 48 },
  { area: "Op. Control", value: 70 },
  { area: "Follow-thru", value: 55 },
  { area: "Team Stability", value: 65 },
];

const TABS = [
  { key: "SCORE", label: "SCORE" },
  { key: "PATTERN", label: "RISK PATTERN" },
  { key: "BLUEPRINT", label: "90-DAY PLAN" },
];

const STATES = [
  {
    eyebrow: "01 / YOUR SCORE",
    headline: "See All Five Dimensions in One View.",
    body: "See where leadership risk is building before it becomes a bigger operating problem.",
  },
  {
    eyebrow: "02 / RISK PATTERN",
    headline: "Your Primary Leadership Risk Pattern.",
    body: "Get a clear answer about which BPO leadership pattern is most likely affecting team performance right now.",
  },
  {
    eyebrow: "03 / 90-DAY PLAN",
    headline: "Your Next Moves, in Order.",
    body: "A practical 90-day action plan tied to BPO team realities and the outcomes you care about most.",
  },
];

const RULER = [
  { day: "30", label: "Identify" },
  { day: "60", label: "Act" },
  { day: "90", label: "Review" },
];
...
const SCORE_PANEL = (
  <div className="flex items-center gap-5">
    <div className="flex flex-col items-center">
      <ScoreGauge value={58} />
      <p className="text-[10px] font-semibold text-gray-500 mt-2 tracking-wide uppercase">BPO Leadership Score</p>
    </div>
...
const PATTERN_PANEL = (
  <div className="space-y-2.5">
    <div className="flex items-center justify-between mb-1">
      <span className="text-[10px] font-bold tracking-wide text-[#0202ff]">#1 RISK PATTERN</span>
      <span className="text-[10px] text-gray-400">PRIMARY</span>
    </div>
    <p className="text-sm font-bold text-[#0a0a0a]">Performance Avoidance</p>
    <p className="text-xs text-gray-500 leading-relaxed">
      Performance issues may be visible, but they are not being addressed quickly or directly enough. In BPO environments, that allows small problems to compound into SLA misses and QA drift.
    </p>
    <div className="flex items-center gap-2 pt-1">
      <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full text-white bg-red-500">Active</span>
      <span className="text-[10px] text-gray-400">Driven by Performance Response · 62/100</span>
    </div>
    <p className="text-[10px] text-gray-400 pt-1">Watch patterns: Coaching Deficit, Accountability Gap</p>
  </div>
);
...
const BLUEPRINT_PANEL = (
  <div className="space-y-2">
    {[
      { day: "30", label: "Identify", action: "Identify where performance concerns are noticed but not acted on quickly enough." },
      { day: "60", label: "Act", action: "Create a simple manager coaching trigger for one recurring issue area." },
      { day: "90", label: "Review", action: "Review whether earlier conversations are reducing repeated team issues." },
    ].map((phase) => (
...
const PANELS = [SCORE_PANEL, PATTERN_PANEL, BLUEPRINT_PANEL];

export default function BpoReportPreviewCard() {
  ...
  return (
    <div className="w-full rounded-xl overflow-hidden border border-gray-200 bg-white shadow-xl">
      ...
      <div className="flex items-center justify-between bg-[#0a0a0a] px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          <div className="text-white">
            ...
            <div className="text-[10px] text-gray-400 leading-none">BPO LEADERSHIP DIAGNOSTIC</div>
          </div>
        </div>
      ...
      <div className="bg-[#0a0a0a] text-white text-center py-2.5 text-xs font-semibold tracking-wide">
        ↓ DOWNLOADABLE 90-DAY ACTION PLAN INCLUDED
      </div>
      ...
      <div className="flex items-start gap-2 px-5 py-3 bg-[#F9F7F5]">
        ...
        <p className="text-[11px] text-gray-500 leading-snug">
          Built from your answers. Ready to print, share, and review with your site or ops leadership.
        </p>
      </div>
    </div>
  );
