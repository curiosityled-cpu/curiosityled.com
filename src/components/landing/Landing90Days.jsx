...
export default function Landing90Days({ industry }) {
  const cfg = industry ? getIndustryConfig(industry) : null;
  const nd = cfg?.ninetyDays;

  const label = nd?.label || "What this makes possible";
  const heading = nd?.heading || "A clearer system for manager development, readiness, and succession.";
  const outcomes = nd?.outcomes || defaultOutcomes;
  const quote = nd?.quote || defaultQuote;
  const image = nd?.image || "/18T115222051.png";
      ...
      <div className="max-w-6xl mx-auto px-6">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          <div>
            ...
            <p className="text-gray-300 text-sm leading-relaxed border-l-2 border-white/30 pl-4 italic">{quote}</p>
          </div>
          <div className="rounded-2xl overflow-hidden">
            <img src={image} alt="Progress and timeline" className="w-full h-full object-cover" />
          </div>
        </div>
      </div>
    </section>
