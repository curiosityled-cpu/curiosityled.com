...
export default function ManagerCheckIn({ promptType = "baseline_energy", onComplete, onDismiss, compact = false, trends, goals, pulses }) {
  const { user } = useAuth();
  ...
  return (
    ...
    >
      ...
      <div className="px-4 pt-4 pb-3 flex items-start justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-sm font-semibold text-foreground leading-snug">{prompt.title}</p>
          </div>
        </div>
        {onDismiss && (
      ...
      <div className="px-4 pb-4 space-y-3">
        {/* Body */}
        <p className="text-sm text-muted-foreground leading-relaxed">{prompt.body}</p>
        ...
        {selected && (
          <div className="space-y-2">
            ...
            <Button
              ...
              onClick={handleDone}
              disabled={saving}
              className="w-full text-xs h-8 bg-foreground hover:bg-foreground/90 text-background rounded-xl"
            >
              {saving ? "Saving…" : "Done"}
            </Button>
          </div>
