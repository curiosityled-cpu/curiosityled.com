...
export default function ManualAssignTestCasesModal({ 
  open, 
  onClose, 
  selectedTestCases,
  onSuccess 
  ...
  const handleAssign = async () => {
    ...
    try {
       ...
       // Assign same tests to all selected users
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => !loading && isOpen === false && onClose()}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>Assign Selected Test Cases</DialogTitle>
        </DialogHeader>
          ...
          <div className="rounded-lg bg-blue-50 p-4 border border-blue-200">
            <div className="flex items-start gap-3">
              ...
              <div className="text-sm">
                ...
                <div className="text-blue-700 text-xs mt-2 space-y-1 max-h-24 overflow-y-auto">
                  {selectedTestCases.slice(0, 3).map(tc => (
                    <div key={tc.id} className="truncate">• {tc.test_case_id}</div>
                  ))}
                  {selectedTestCases.length > 3 && (
                    <div className="text-blue-600 font-medium">+ {selectedTestCases.length - 3} more</div>
                  )}
                </div>
              </div>
            </div>
          ...
          <div className="rounded-lg bg-amber-50 p-3 border border-amber-200">
            <div className="flex items-start gap-2">
              ...
              <p className="text-xs text-amber-700">
                All selected test cases will be assigned to each selected user
              </p>
            </div>
          </div>
        </div>
        ...
        <DialogFooter>
           ...
           <Button 
             ...
             disabled={selectedUsers.length === 0 || loading || selectedTestCases.length === 0}
