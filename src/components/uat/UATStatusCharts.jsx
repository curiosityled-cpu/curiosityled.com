...
export default function UATStatusCharts({ testCases }) {
  ...
  const { pieData, completedTests, completionPercentage, completionGoal } = useMemo(() => {
    if (!testCases || testCases.length === 0) {
      return { pieData: [], completedTests: 0, completionPercentage: 0, completionGoal: 80 };
    }

    const getLatestTestRun = (testCase) =>
      testCase.test_runs && testCase.test_runs.length > 0
        ? testCase.test_runs[testCase.test_runs.length - 1]
        : null;

    const counts = { 'Not Tested': 0, 'Passed': 0, 'Failed': 0, 'Blocked': 0 };
    ...
    return { pieData: pieChartData, completedTests: completed, completionPercentage: percentage, completionGoal: 80 };
  }, [testCases]);

  if (!testCases || testCases.length === 0) {
    return (
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card><CardContent className="py-12 text-center text-gray-500"><p className="text-sm">No test cases available</p></CardContent></Card>
        <Card><CardContent className="py-12 text-center text-gray-500"><p className="text-sm">No data to display</p></CardContent></Card>
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <Card>
        <CardHeader><CardTitle className="text-lg">Test Status Distribution</CardTitle></CardHeader>
        <CardContent>
          <ResponsiveContainer width="100%" height={350}>
            <PieChart>
              <Pie data={pieData} cx="50%" cy="50%" labelLine={false} label={({ name, value }) => `${name}: ${value}`} outerRadius={80} dataKey="value" animationBegin={0} animationDuration={300}>
                {pieData.map((entry, index) => (
                  <Cell key={`cell-${index}`} fill={statusColors[entry.name]} />
                ))}
              </Pie>
              <Tooltip formatter={(value) => `${value} test(s)`} contentStyle={{ backgroundColor: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '0.5rem' }} cursor={{ fill: 'rgba(0,0,0,0.05)' }} />
      ...
      <Card>
        <CardHeader><CardTitle className="text-lg">Completion Progress</CardTitle></CardHeader>
        <CardContent className="space-y-6">
          <div>
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm font-medium text-gray-700">Tests Executed</span>
              <span className="text-sm font-bold text-gray-900">{completedTests} / {testCases.length}</span>
            </div>
