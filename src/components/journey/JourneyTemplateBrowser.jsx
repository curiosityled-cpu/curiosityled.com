...
import { Search, Map, Clock, TrendingUp, Filter, X, BookOpen, Route, List } from "lucide-react";
...
export default function JourneyTemplateBrowser({ isOpen, onClose, onSelectTemplate }) {
  ...
  const loadTemplates = async () => {
    setLoading(true);
    try {
      const allJourneys = await base44.entities.LearningJourney.filter({ is_template: true });
      setTemplates(allJourneys);
    } catch (error) {
  ...
  const handleSelectTemplate = async (template) => {
    try {
      await base44.entities.LearningJourney.update(template.id, {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Browse and select reusable learning journey templates
          </DialogDescription>
        </DialogHeader>
                  ...
                  >
                    <CardContent className="p-4">
                      <div className="flex items-start justify-between mb-3">
                        ...
                        {template.type === 'learning_path' ? (
                          <Route className="w-5 h-5 text-purple-500 flex-shrink-0 ml-2" />
                        ) : (
                          <List className="w-5 h-5 text-blue-500 flex-shrink-0 ml-2" />
                        )}
                      </div>
                      ...
                      <div className="flex items-center gap-2 mb-3 flex-wrap">
                        ...
                        <Badge variant="outline" className="text-xs">
                          {template.type === 'curriculum' ? 'Curriculum' : 'Learning Path'}
                        </Badge>
                        ...
                        {template.content_structure?.length > 0 && (
                          <Badge variant="outline" className="text-xs flex items-center gap-1">
                            <BookOpen className="w-3 h-3" />
                            {template.content_structure.length} resources
                          </Badge>
                        )}
                      </div>
