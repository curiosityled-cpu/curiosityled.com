...
function ShareEmailDialog({ open, onClose, user, assessment, report }) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const archetype = report?.archetype || assessment?.archetype_label || "—";
      const band = assessment?.band_overall || getBand(overall);
      const dna = report?.leadership_dna?.description || "";
...
<div style="font-family:sans-serif;max-width:640px;margin:auto;padding:32px;border:1px solid #e5e7eb;border-radius:12px;">
  <div style="background:linear-gradient(to right,#0012ff,#3b30ff);border-radius:10px;padding:24px 28px;margin-bottom:24px;">
    ...
    <p style="color:#c7d2fe;font-size:13px;margin:0;">${user?.current_role || ""}${user?.sector ? " · " + user.sector : ""}</p>
  ...
  ${dna ? `<div style="background:#f0f4ff;border-left:4px solid #0012ff;padding:14px 18px;border-radius:6px;margin-bottom:20px;font-size:14px;color:#374151;font-style:italic;">"${dna}"</div>` : ""}
  ...
  <table style="width:100%;border-collapse:collapse;font-size:13px;margin-bottom:20px;">
    ...
    ${COMP_KEYS.map(({ key, full }) => {
      const score = assessment?.[key];
      if (score == null) return "";
      return `<tr><td style="padding:8px 12px;border-top:1px solid #f3f4f6;">${full}</td><td style="padding:8px 12px;border-top:1px solid #f3f4f6;text-align:right;font-weight:600;color:#0012ff;">${score}%</td><td style="padding:8px 12px;border-top:1px solid #f3f4f6;text-align:right;color:#6b7280;">${getBand(score)}</td></tr>`;
    }).join("")}
  </table>

  ${report?.blind_spots?.primary ? `
  ...
  <div style="background:#fff7ed;border:1px solid #fed7aa;border-radius:8px;padding:14px 18px;margin-bottom:20px;">
    <p style="font-weight:600;color:#c2410c;margin:0 0 6px;">${report.blind_spots.primary.title}</p>
    <p style="font-size:13px;color:#374151;margin:0;">${report.blind_spots.primary.why_it_happens}</p>
  </div>` : ""}
