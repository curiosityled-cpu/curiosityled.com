  ...
  return (
    <Card className="border-0 shadow-lg">
      ...
      <CardContent className="space-y-5">
        ...
        {/* Leadership Index Score — Radar */}
        <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-4">
          <div className="flex items-center justify-between mb-1">
            <span className="text-sm font-semibold text-indigo-900">Leadership Index Score</span>
          ...
          <div className="flex items-center gap-1 text-xs text-indigo-600 mb-3">
            ...
            <span>Decision Quality Score: <strong>{decisionQuality}%</strong></span>
          </div>

          {/* Radar legend */}
          <div className="flex items-center gap-4 mb-2 text-[10px] text-gray-500">
            <span className="flex items-center gap-1"><span className="inline-block w-5 border-t-2 border-indigo-600" /> Your Score</span>
            <span className="flex items-center gap-1"><span className="inline-block w-5 border-t-2 border-dashed border-amber-400" /> Industry Benchmark</span>
          </div>

          <ResponsiveContainer width="100%" height={220}>
            <RadarChart data={radarData} margin={{ top: 5, right: 20, bottom: 5, left: 20 }}>
              <PolarGrid stroke="#c7d2fe" />
              <PolarAngleAxis dataKey="subject" tick={{ fontSize: 11, fill: "#4338ca", fontWeight: 600 }} />
          ...
          {/* Score grid */}
          <div className="grid grid-cols-3 gap-x-4 gap-y-1 mt-2 border-t border-indigo-200 pt-3">
            {scoreRows.map(({ key, score, target }) => (
              <div key={key} className="flex items-baseline gap-1 text-xs">
                ...
                <span className="text-gray-400 text-[10px]">/ {target}% target</span>
              </div>
            ))}
          </div>
        </div>

        {/* Two-column: Flight Risk + Learning Velocity */}
          ...
          <div>
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex items-center gap-1.5">
                <span className={`text-lg font-bold ${learningRate >= 70 ? 'text-green-600' : learningRate >= 50 ? 'text-yellow-600' : 'text-red-600'}`}>{learningRate}%</span>
                <Badge className={velocityColor}>{velocityLabel}</Badge>
              </div>
            </div>
            <div className="space-y-1 text-xs">
              <div className="flex justify-between text-gray-600">
                <span className="flex items-center gap-1"><span className="w-2 h-2 bg-green-400 rounded-full inline-block" />Completed</span>
                <span className="font-medium">{completedLearning}</span>
              </div>
              <div className="flex justify-between text-gray-600">
                <span className="flex items-center gap-1"><span className="w-2 h-2 bg-gray-300 rounded-full inline-block" />Remaining</span>
                <span className="font-medium">{incompleteLearning}</span>
              </div>
              <div className="flex justify-between text-gray-600">
                <span className="flex items-center gap-1"><span className="w-2 h-2 bg-blue-400 rounded-full inline-block" />Total assigned</span>
              ...
              {allRisks.map((risk, idx) => (
                ...
                >
                  <div className="flex items-center gap-1.5 mb-1">
                    ...
                    <span className="text-xs font-semibold text-red-800">{risk.title}</span>
                  </div>
                  {risk.description && <p className="text-xs text-red-700 leading-tight">{risk.description}</p>}
              ...
              {allOpps.map((opp, idx) => (
                ...
                >
                  <div className="flex items-center gap-1.5 mb-1">
                    ...
                    <span className="text-xs font-semibold text-green-800">{opp.title}</span>
                  </div>
                  {opp.description && <p className="text-xs text-green-700 leading-tight">{opp.description}</p>}
