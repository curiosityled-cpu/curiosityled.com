...
export default function CoachingRequestTriageBoard() {
  const { user } = useAuth();
  ...
  const requestsByStatus = useMemo(() => {
    ...
    requests.forEach(r => {
      if (r.status === "intake_complete") map["approved"]?.push(r);
      else if (r.status === "in_progress") map["engagement_created"]?.push(r);
      else if (r.status === "accepted" || r.status === "declined") map["assigned"]?.push(r);
