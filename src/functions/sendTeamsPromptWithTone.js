 ...
 * Includes "Why I asked this" explanation inline
 */
...
Deno.serve(async (req) => {
  try {
    ...
    // Adapt prompt language based on tone mode
    const adaptedPrompts = {
      gentle_observant: {
        operator_mode: `I've noticed a pattern over the last few days: high load combined with tighter control. This can be a sign of stress. How are you feeling about delegating today?`,
        learning_stall: `It's been quiet on the learning front lately. No pressure—just checking in: is there something specific you'd like to explore?`,
      ...
      close_friend_candid: {
        operator_mode: `Dude, you're doing the thing again—taking on too much and micromanaging. What's actually going on?`,
        learning_stall: `You haven't done any learning in a week. You know you need this. What's blocking you?`,
    ...
    // Build explanation for "Why I asked this"
    const explanations = {
    ...
    // Call sendTeamsPrompt with tone-aware message
    const result = await base44.functions.invoke('sendTeamsPrompt', {
      user_email,
