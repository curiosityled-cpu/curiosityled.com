 ...
 * 
 * Scheduled Mon-Fri 8am EST. Sends a Teams prompt asking "What matters most today?"
...
Deno.serve(async (req) => {
  try {
    ...
    // Fetch tone preference for this user
    ...
    // Check cadence preference (skip if paused or important_only)
    if (tonePref.cadence_preference === 'paused') {
      return Response.json({ skipped: true, reason: 'cadence paused' });
    }

    // Check last prompt sent (throttle to prevent spam)
    const lastPromptSent = tonePref.last_prompt_sent_at
      ? new Date(tonePref.last_prompt_sent_at)
      : null;
    const now = new Date();
    const hoursSinceLastPrompt = lastPromptSent
      ? (now - lastPromptSent) / (1000 * 60 * 60)
      : null;
    ...
    const promptText = toneText[tonePref.tone_mode] || toneText.warm_candid;

    // Call sendTeamsPrompt to deliver the adaptive card
    const promptResponse = await base44.functions.invoke('sendTeamsPrompt', {
      user_email,
      title: 'Morning Focus',
      message: promptText,
    ...
    await base44.entities.TonePreference.update(tonePref.id, {
